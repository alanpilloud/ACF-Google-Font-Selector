(function($) {
  ...
  if (typeof acf.add_action !== 'undefined') {
    ...
    $(document).live('acf/setup_fields', function(e, postbox) {
      $(postbox)
        .find('.field[data-field_type="google_font_selector"]')
        .each(function() {
          initialize_field($(this));
          jQuery(document).on(
            'change',
            '.acfgfs-font-family select',
            function() {
              var new_font = $(this).val();
              var container = $(this).parents('.acfgfs-font-selector:first');
              var variants = container.find(
                '.acfgfs-font-variants .acfgfs-list',
              );
              var subsets = container.find('.acfgfs-font-subsets .acfgfs-list');
              var data = container.find('.acfgfs-font-data').val();
              jQuery.ajax({
                  ...
                  container
                    ...
                    .html(
                      '<link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=' +
                        ...
                        '"><div style="font-family:' +
                        font +
                        '">This is new a preview of the selected font</div>',
                    );
                },
              });
            },
